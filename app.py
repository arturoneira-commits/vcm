...
# Carga segura de BD Innovación
try:
    if os.path.exists('BD Innovacion.xlsx') and len(st.session_state.df_bd) == 0:
        st.session_state.df_bd = pd.read_excel('BD Innovacion.xlsx', sheet_name='Hoja1')
        st.session_state.df_bd.columns = [str(c).strip() for c in st.session_state.df_bd.columns]
except Exception as e:
    pass

# Carga segura de Incubadoras
try:
    inc_files = [f for f in os.listdir('.') if 'INC' in f.upper() and f.endswith('.xlsx')]
...
# Carga segura de PAC
try:
    pac_files = [f for f in os.listdir('.') if 'PAC' in f.upper() and f.endswith('.xlsx')]
...
if app_mode == "📊 Dashboard Principal":
    ...
    if dataset_choice == "BD Innovación":
        ...
        if not df_bd.empty:
            ...
            st.info("No se encontró el archivo 'BD Innovacion.xlsx' en el repositorio.")
        ...
        if dict_inc:
            ...
            st.info("No se encontró el archivo de Incubadoras en el repositorio.")
        ...
        if 'Proyectos' in dict_pac:
            ...
            st.info("Sube o verifica el archivo 'reporte_general_PAC (6).xlsx' en la raíz de tu repositorio de GitHub.")
